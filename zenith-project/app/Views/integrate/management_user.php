...
<div class="sub-contents-wrap db-manage-contaniner">
    <div class="title-area">
        <h2 class="page-title">사용자 DB 관리</h2>
    ...
    <div class="section client-list custom-margin-box-1" id="advertiserBtn">
...
$.fn.DataTable.Api.register('buttons.exportData()', function (options) { //Serverside export
    var arr = [];
    $.ajax({
        "url": "<?=base_url()?>/integrate-user/list",
...
function getMemoList(seq) { //메모 수신
    $.ajax({
        type: "get",
        url: "<?=base_url()?>integrate-user/getmemo",
        data: {'seq': seq},
        dataType: "json",
...
function registerMemo(data) { //메모 등록
    $.ajax({
        type: "post",
        url: "<?=base_url()?>integrate-user/addmemo",
        data: data,
        dataType: "json",
...
function setStatus(t) {
    ...
    $.ajax({
        type: "post",
        url: "<?=base_url()?>/integrate-user/setstatus",
        data: data,
        dataType: "json",
        success: function(response){  
            if(response.result == true) {
