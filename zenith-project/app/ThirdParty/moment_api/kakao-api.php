...
class ChainsawKM
{
    ...
    */
    ...
    { //유효DB 개수 업데이트
        ...
        $creatives = $this->db->getCreativeReportBasic("AND report.date='{$date}' GROUP BY report.id");
        $step = 1;
        $total = $creatives->getNumRows();
        if(!$total) return null;
        $result = [];
        ...
        foreach ($creatives->getResultArray() as $row) {
            ...
            */
            ...
            if(!$data['event_seq'] && $data['media']) {
                if($data['period_ad']) $period_margin = $data['cost'] * ('0.' . $data['period_ad']);
                $data['data'][] = ['hour' => "",'count' => "",'sales' => "",'margin' => $period_margin];
            }
            $initZero = false;
            ...
            if(!is_null($leads)) {
                foreach($leads->getResultArray() as $row) {
                    ...
                    if($period_margin !== 0) $margin = $period_margin;
                    $data['data'][] = [
                        'hour' => $row['hour']
                        ,'count' => $db_count
                        ,'sales' => $sales
